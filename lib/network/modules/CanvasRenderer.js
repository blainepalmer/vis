...
let C2S = require('canvas2svg-visjs');
...
class CanvasRenderer {
  ...
  /**
   * Output serialized SVG from canvas contents
   */
  serializeCanvasToSvg(embedImages = false) {
    this.body.emitter.emit('setSize');
    return this._serializeCanvasToSvg(embedImages);
  }
  ...
  _redraw(hidden = false) {
    if (this.allowRedraw === true) {
      ...
      this._drawOnCanvasContext(ctx, w, h, hidden);
    }
  }

  _serializeCanvasToSvg(embedImages = false) {
    let w = this.canvas.frame.canvas.clientWidth;
    let h = this.canvas.frame.canvas.clientHeight;
    ...
    let ctx = new C2S({width: w, height: h, enableMirroring: embedImages});

    this._drawOnCanvasContext(ctx, w, h);

    return ctx.getSerializedSvg();
  }

  _drawOnCanvasContext(ctx, w, h, hidden = false) {
